...
class Losses:
    ...
    def __call__(self, value):
        if self.fn_name == 'squared_error':
            ...
            return self.margin_ranking(value, self.goal, self.margin)
    
    def squared_error(self, a, b):
        device = b.device
        return (a-b) #.pow(2).sqrt()
    
    def margin_ranking(self, x1, x2=0.0, margin = 0.0):
        device = x1.device
        return torch.max(torch.tensor(0.0, device=device), (x1 - x2) + margin) 
